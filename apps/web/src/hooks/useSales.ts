...
export function useCollectSale() {
  const qc = useQueryClient();
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: { driverName?: string; collectionCarrierVolumes?: number; employeeCode?: string; carrierId?: string; shippingCost?: number; freightCurrency?: string; freightMode?: string; trackingCode?: string; deliveryForecast?: string } }) => {
